...
TEST_CASE("Throws Check")
{
    ...
    for (size_t i = 0; i < testcount; i++)
    {
        ...
        unsigned int col = (unsigned int)rand();
        col %= 100;
        unsigned int row = (unsigned int)std::rand();
        unsigned int page = (unsigned int)std::rand();

        if (col >= 96)
        {
            CHECK_THROWS_MESSAGE(notebook.write(row, col, page, Direction::Horizontal, "Test"), "trying to write line from col index exceed line size.\n");
        }
        ...
        else
        {
            try
            {
                notebook.write(row, col, page, Direction::Horizontal, "Test");
                CHECK_EQ(notebook.read(row, col, page, Direction::Horizontal, 4), "Test");
            }
